<template>
  <div class="cart card p-3">
    <!-- عنوان الكارت وعدد العناصر -->
    <h2 class="fw-bold mb-3" style="color: var(--color-red);">
 ...
 <div style="display: flex; flex-direction: row; gap: 8px; align-items: center;">
  ...
  <span style="color: var(--color-rose-400);">
  ...
  <span class="fw-bold me-3" style="color: var(--color-rose-500);">
    ${{ (item.price * item.quantity).toFixed(2) }}
      ...
      <!-- زر التأكيد -->
      <button class=" confirm-btn btn btn-danger w-100 mt-3 ">
        Confirm Order
      </button>
    </div>
  </div>
...
.delete-btn {
  border: 1px solid #CAAFA7; /* نفس لون الأيقونة */
...
.delete-btn svg path {
  fill: #CAAFA7; /* نفس اللون الأساسي */
  transition: fill 0.2s ease-in-out;
}

.delete-btn:hover {
  border-color: #000; /* البوردر يصير أسود */
}

.delete-btn:hover svg path {
  fill: #000; /* الأيقونة تصير أسود */
}
.confirm-btn {
